...
int main(void)
{
  ...
  HAL_UART_Receive_IT(&huart3, &rx_data, sizeof(rx_data)); // uart中断
  HAL_ADCEx_Calibration_Start(&hadc1, ADC_SINGLE_ENDED);   // adc1自校准
  HAL_ADCEx_Calibration_Start(&hadc2, ADC_SINGLE_ENDED);   // adc2自校准
  TIM1->ARR = 8000 - 1;
  TIM1->CCR4 = 8000 - 2;
