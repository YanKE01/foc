...
void current_loop_step(void)
{
  ...
  if (rtDW.is_active_c7_current_loop == 0U) {
    rtDW.is_active_c7_current_loop = 1U;
    rtDW.is_c7_current_loop = IN_IDLE;
  } else {
    switch (rtDW.is_c7_current_loop) {
     case IN_AlignStage:
      if (rtDW.temporalCounter_i1 >= 1000) {
        rtDW.temporalCounter_i1 = 0U;
        rtDW.is_c7_current_loop = IN_OpenStage;
        rtDW.ZReset = 0.0F;
        rtDW.cnt = 0.0F;
     ...
     case IN_IDLE:
      if (rtU.on_off == 1.0) {
        rtDW.temporalCounter_i1 = 0U;
        rtDW.is_c7_current_loop = IN_AlignStage;
      } else {
     ...
     case IN_OpenStage:
      if (rtU.on_off == 0.0) {
        ...
        rtDW.temporalCounter_i1 = 0U;
        rtDW.is_c7_current_loop = IN_ThetaAlign;
      } else {
        if (rtDW.cnt == 1.0F) {
          rtDW.ZReset = 1.0F;
        }
     ...
     default:
      ...
      if (rtDW.temporalCounter_i1 >= 5000) {
        rtDW.is_c7_current_loop = IN_RunStage;
      } else {
  ...
  if (rtM->Timing.TaskCounters.TID[1] == 0) {
     ...
     */
    rtb_DeadZone_b = 0.010389F * rtb_RateTransition1 + Integrator;

    /* Saturate: '<S221>/Saturation' incorporates:
     *  DeadZone: '<S207>/DeadZone'
     */
    if (rtb_DeadZone_b > 5.0F) {
  ...
  switch ((int32_T)rtDW.Motor_state) {
   ...
   case 3:
    ...
    rtDW.DiscreteTimeIntegrator_DSTATE_m += 0.00418879045F;
    if (rtDW.ZReset > 0.0F) {
   ...
   case 4:
    ...
    rtDW.DiscreteTimeIntegrator1_DSTAT_e += 0.0125663709F;
   ...
   *  Sum: '<S12>/Add'
   */
  rtb_Sum = 0.0 - (rtb_Add1_j * rtb_Cos + rtb_Gain2_c * rtb_Saturation_k);
  ...
  if (rtDW.obj.TunablePropsChanged) {
    ...
    flag = (rtDW.obj.pStatistic->isInitialized == 1);
    if (flag) {
      rtDW.obj.pStatistic->TunablePropsChanged = true;
    }

    rtDW.obj.pStatistic->ForgettingFactor = 0.99F;
  }
...
void current_loop_initialize(void)
{
  /* Registration code */
...

  {
    ...
    /* Enable for Atomic SubSystem: '<Root>/current_loop' */
    ...
    /* End of Enable for SubSystem: '<Root>/current_loop' */
  }
}
